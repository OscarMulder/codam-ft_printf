...
/*   Updated: 2019/02/12 14:36:23 by omulder       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int		printed_chars(t_fmt *fmt, short num)
{
	int	ilen;
...
static void		put_paddingandsign(t_fmt *fmt, short num, int ilen)
{
...
static void		put_backpadding(t_fmt *fmt, short num, int ilen)
{
	if (fmt->opt[2])
	{
		if (fmt->opt[0] && (fmt->conv == 'x' || fmt->conv == 'X') && num != 0)
