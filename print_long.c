...
/*   Updated: 2019/02/11 18:23:27 by omulder       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int		printed_chars(t_fmt *fmt, long long num)
{
	int	ilen;
...
static void		put_paddingandsign(t_fmt *fmt, long long num, int ilen)
{
...
static void		put_backpadding(t_fmt *fmt, long long num, int ilen)
{
	if (fmt->opt[2])
	{
		if (fmt->opt[0] && (fmt->conv == 'x' || fmt->conv == 'X') && num != 0)
